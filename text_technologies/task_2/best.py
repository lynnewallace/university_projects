...
for query in qrys:
  ...
  for doc in docs:
    ...
    # add to document score dictionary
    x[weighted_sum] = doc[1:]

  # add top 7 documents to the query
  top = sorted(x.iteritems(), reverse=True)[:relevant]
  for i in top:
    query.extend(i[1])
